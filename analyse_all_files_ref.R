...
product <- "reference" #OR product <- "pngase"  #OR product <- "sample" 
...
#                            filename,
#                            delim = "_",
#                            names = c("day","month", "year", "experiment", "technical_replicate","aquisition_number"),
#                            too_many = "debug") %>%
...
# 
# write_csv(x = df,
#           file = "data/20240209_Nistmab_firstMeasurements/overview_reference.csv")

# import data with information on charge states and rt limits  --------
cs_rt_data <- read_csv('data/20240209_Nistmab_firstMeasurements/rt_seconds_ref_mab.csv')
...
data_merged <- df %>% 
  select(mzml_full_path, filename, analysis_path) %>%
  left_join(cs_rt_data, by = join_by(filename == sample_name))
  #cs_rt_data %>%  select(CHO_cell_variant_bio_replicate, )
  # filter(CHO_cell_variant.x != "A25") # nearly no signal

write_csv(x = data_merged,
          file = "data/20240209_Nistmab_firstMeasurements/overview_reference_merged.csv")
