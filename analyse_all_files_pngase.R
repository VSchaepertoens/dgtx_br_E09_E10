...
product <- "pngase" #OR product <- "pngase"  #OR product <- "sample" 

directory <- "data/20240209_Nistmab_firstMeasurements/pngase/"
...
df <- tibble(mzml_full_path = dir_ls(path = directory,regexp = pattern),) %>%
  separate(mzml_full_path,
           into = c("data", "exp_month","subdir","filename"),
           sep = "/",
           remove = FALSE) %>%
...
# write_csv(x = df,
#           file = "data/20240209_Nistmab_firstMeasurements/overview_cpb_pngase.csv")

# import data with information on charge states and rt limits  --------

cs_rt_data <- read_csv('data/20240209_Nistmab_firstMeasurements/rt_seconds_cpb_pngase.csv') %>%
  separate(sample_name,
           into = c("day", "month", "year", "experiment","technical_replicate","enzyme", "enzyme2","aquisition_number"),
           sep = "_",
           remove = FALSE) %>%
...
write_csv(x = data_merged,
          file = "data/20240209_Nistmab_firstMeasurements/overview_cpb_pngase_merged.csv")
